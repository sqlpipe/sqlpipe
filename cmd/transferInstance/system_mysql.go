...
func newMysql(connectionInfo ConnectionInfo) (mysql Mysql, err error) {
	...
	connectionString := fmt.Sprintf("%v:%v@tcp(%v:%v)/%v?parseTime=true&loc=US%%2FPacific", connectionInfo.Username,
		connectionInfo.Password, connectionInfo.Hostname, connectionInfo.Port, connectionInfo.Database)

	db, err := openConnectionPool(connectionString, DriverMySQL)
	if err != nil {
...
func (system Mysql) createTableIfNotExistsOverride(schema, table string, transferInfo *data.TransferInfo) (overridden bool, err error) {
	return false, nil
}
...
func (system Mysql) pipeTypeToCreateType(columnInfo *data.ColumnInfo) (createType string, err error) {
...
func (system Mysql) createPipeFilesOverride(pipeFileChannelIn chan PipeFileInfo, transferInfo *data.TransferInfo, rows *sql.Rows,
...
func (system Mysql) insertPipeFilesOverride(transferInfo *data.TransferInfo, pipeFileInfoChannel <-chan PipeFileInfo) (overridden bool, err error) {
	return false, nil
}

func (system Mysql) convertPipeFilesOverride(pipeFilePath <-chan PipeFileInfo, finalCsvInfoChannelIn chan FinalCsvInfo, transferInfo *data.TransferInfo) (finalCsvInfoChannel chan FinalCsvInfo, overridden bool) {
...
func (system Mysql) listAllDatabases() (databases []string, err error) {
...
func (system Mysql) listAllTablesInDatabase(database string) (tables []string, err error) {
	rows, err := system.query(fmt.Sprintf(`SELECT table_name
		FROM information_schema.tables
		WHERE table_schema = '%v'
		AND table_type = 'BASE TABLE'`, database))
	if err != nil {
		return nil, fmt.Errorf("error listing all tables in database :: %v", err)
	}
...
func (system Mysql) createDbIfNotExistsOverride(database string) (overridden bool, err error) {
	return false, errors.New("not implemented")
}

func (system Mysql) discoverStructure(instanceTransfer *data.InstanceTransfer) (*data.InstanceTransfer, error) {
	...
	for _, database := range databases {
		...
		dbSystem, err := newMysql(dbConnInfo)
		if err != nil {
		...
		targetSchemaTemplate := instanceTransfer.NamingConvention.SchemaFallbackInSnowflake
		targetTableTemplate := instanceTransfer.NamingConvention.TableNameInSnowflake

		tables, err := dbSystem.listAllTablesInDatabase(database)
		if err != nil {
			return nil, fmt.Errorf("error listing all tables in database %v :: %v", database, err)
		}
		for _, table := range tables {
			treeNodeTableID := fmt.Sprintf("%v_%v", database, table)

			tableNode := dbNode.AddChild(treeNodeTableID, table)
