...
# COPY build/mysql-connector-odbc_8.0.30-1ubuntu22.04_amd64.deb /
# COPY build/mysql-apt-config_0.8.23-1_all.deb /
# COPY build/mysql-community-client-plugins_8.0.30-1ubuntu22.04_amd64.deb /
...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 467B942D3A79BD29 \
  && echo "deb http://repo.mysql.com/apt/ubuntu/ jammy  mysql-8.0" > /etc/apt/sources.list.d/mysql.list \
  && apt-get update \
  && apt-get install -y libodbc2 libodbcinst2 mysql-community-client-plugins \
  && curl -O http://repo.mysql.com/apt/ubuntu/pool/mysql-tools/m/mysql-connector-odbc/mysql-connector-odbc_8.0.30-1ubuntu22.04_amd64.deb \
  && dpkg -i mysql-connector-odbc_8.0.30-1ubuntu22.04_amd64.deb \
  && rm mysql-connector-odbc_8.0.30-1ubuntu22.04_amd64.deb
