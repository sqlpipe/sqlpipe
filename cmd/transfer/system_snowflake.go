...
func (system Snowflake) insertPipeFilesOverride(columnInfos []ColumnInfo, transferInfo data.TransferInfo, pipeFileInfoChannel <-chan PipeFileInfo, vacuumTable string) (overridden bool, err error) {
	...
	err = insertFinalCsvs(putCsvsChannel, transferInfo, system, transferInfo.TargetSchema, table)
	if err != nil {
...
func (system Snowflake) putCsvs(
	...
	go func() {
		...
		for finalCsvInfo := range finalCsvChannelIn {
			...
			escapedSchema := escapeIfNeeded(transferInfo.TargetSchema, system)
			...
			if err != nil {
				transferInfo.Error = fmt.Sprintf("error putting csv into snowflake :: %v", err)
				logger.Error(fmt.Sprintf(transferInfo.Error))
				return
			}
